...
export function ResearchWorkspaceView({ workspace, confidence }: { workspace: ResearchWorkspace; confidence?: number | null }) {
  ...
  const ratioMetrics: Metric[] = [
    { label: "Debt / equity (quarterly)", value: formatMetric(facts.balanceSheet?.debtToEquityQuarterly) },
    { label: "Current ratio (quarterly)", value: formatMetric(facts.balanceSheet?.currentRatioQuarterly) },
