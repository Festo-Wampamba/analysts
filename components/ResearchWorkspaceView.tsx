...
export function FactLabel({ children, tone = "fact" }: { children: React.ReactNode; tone?: "fact" | "ai" | "fallback" }) {
  return <span className={`eyebrow eyebrow--${tone}`}>{tone === "fact" ? <SearchIcon className="chip-icon" /> : <SparkIcon className="chip-icon" />}{children}</span>;
}
...
function AiPanel({ children, fallback }: { children: React.ReactNode; fallback: boolean }) {
  return <Panel className="ai-panel">{fallback ? <FactLabel tone="fallback">Fallback · sourced data only</FactLabel> : <FactLabel tone="ai">AI-generated</FactLabel>}{children}</Panel>;
}
  ...
  return (
    <main className="research-layout" id="research">
      ...
      <article className="report-column">
        ...
        <section className="report-section" id="cases"><SectionHeading id="cases" number="07" title="Bull / Base / Bear" /><div className="scenario-grid">{narrative.scenarios.map((scenario) => <Panel className={`scenario-card scenario-card--${scenario.label}`} key={scenario.label}>{fallback ? <FactLabel tone="fallback">Fallback · {scenario.label}</FactLabel> : <FactLabel tone="ai">AI-generated · {scenario.label}</FactLabel>}<div className="scenario-price"><strong>{scenario.label}</strong></div><p>{scenario.summary}</p><small>Qualitative scenario · no model-generated target</small></Panel>)}</div></section>

        <section className="report-section" id="thesis"><SectionHeading id="thesis" number="08" title="Investment thesis" /><Panel className="thesis-panel"><div className="thesis-head">{fallback ? <FactLabel tone="fallback">Fallback · sourced data only</FactLabel> : <FactLabel tone="ai">AI-generated</FactLabel>}{confidence !== undefined && confidence !== null && <span>coverage confidence {confidence.toFixed(2)}</span>}</div>{confidence !== undefined && confidence !== null && <div className="confidence-bar"><span style={{ width: `${Math.max(0, Math.min(1, confidence)) * 100}%` }} /></div>}<p>{narrative.thesis}</p><small>Model: {generated.modelLabel ?? "unlabelled"} · sourced facts and generated prose remain separately identified</small></Panel></section>
