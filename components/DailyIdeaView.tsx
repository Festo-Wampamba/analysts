...
export function DailyIdeaView({
  ...
  const renderCandidateRow = (candidate: LatestIdea["candidates"][number]) => {
    ...
    return <div className="candidate-row" key={candidate.ticker}><strong><a href={`/research/${candidate.ticker}`}>{candidate.ticker}</a></strong><span>{candidate.compositeScore.toFixed(2)}</span><span>{candidate.sector ?? "—"}</span><div className="candidate-leading">{factor && <FactorScoreChip label={factor.label} score={factor.score} />}<span>{candidate.leadingEvidence}</span></div><strong className={candidate.compositeScore >= latest!.threshold ? "trend-up" : "trend-neutral"}>{candidate.coverage === undefined ? "—" : `${(candidate.coverage * 100).toFixed(0)}%`}</strong></div>;
  };
      ...
      {hasPick ? (
        <section className="daily-pick">
          ...
          <div className="daily-evidence-grid">
            ...
            <Panel className="daily-evidence-card"><FactLabel tone="ai">Catalyst, cases &amp; risks</FactLabel><div className="daily-narrative-detail"><div><span>Recent catalyst</span><p>{idea.narrative.keyCatalyst}</p></div><div><span>Bull case</span><p>{idea.narrative.bullCase}</p></div><div><span>Bear case</span><p>{idea.narrative.bearCase}</p></div><div><span>Principal risks</span><ul>{idea.narrative.risks.map((risk) => <li key={risk}>{risk}</li>)}</ul></div></div></Panel>
            <Panel className="daily-evidence-card"><FactLabel>Screen · Confidence &amp; sources</FactLabel><div className="daily-confidence"><strong>{(idea.facts.coverage * 100).toFixed(0)}% coverage confidence</strong><p>{idea.narrative.confidenceRationale}</p></div><div className="daily-source-links"><span>{dailySourceProviders.length ? `${dailySourceProviders.join(" · ")} sources` : "Provider sources"}</span><a href="#sources">Open report source audit</a>{idea.facts.news?.slice(0, 2).map((item) => <a href={item.url} target="_blank" rel="noreferrer" key={item.url}>{item.source}: {item.headline}</a>)}</div></Panel>
          </div>
